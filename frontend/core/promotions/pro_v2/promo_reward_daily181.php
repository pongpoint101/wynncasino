...
$sql = "SELECT 1 FROM log_deposit WHERE  member_no=? AND promo=181 AND trx_date=SUBDATE(CURDATE(),0);";
...
if ($res->num_rows() > 0) {
  ...
  $sql = "SELECT * FROM rewardmaxwinner_player_daily WHERE member_no=? AND trx_date = SUBDATE(CURDATE(),2) AND status=1 ORDER BY id DESC LIMIT 1;"; 
  $res = $conn->query($sql,[$member_no]);
  if ($res->num_rows() <= 0) {
    $errCode = 402;
