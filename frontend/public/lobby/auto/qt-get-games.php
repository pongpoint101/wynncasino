...
//     $sql = "SELECT * FROM qt_game_list WHERE provider_id='{$_GET['provider']}'";
// } else {
//     $queryParam = 'gameTypes=' . $_GET['gt'];
//     $sql = "SELECT * FROM qt_game_list";
//     exit();
// }
...
$cat = "BINGO,CASUALGAME,ESPORTS,INSTANTWIN,LIVECASINO,SCRATCHCARD,SHOOTING,SLOT,SPORTS,TABLEGAME,VIDEOPOKER,VIRTUAL_SPORTS,LOTTERY";
$pv = "1x2,AUX,BBG,BTG,BPG,BNG,CQC,DS,ELK,EVP,FNG,FUG,GA,GFG,GZX,HAV,HAK,IDS,KGL,MNP,MAV,MOB,NE,NGE,NLC,OMI,OT,PNG,PP,PPC,PRS,PRG,PUG,QS,RED,RLX,RLG,RG,RTG,SWC,SM,SHS,SPR,TK,TPG,WAZ,WOO,YGG,ODN";
$pvArr = explode(',', $pv);
$queryParam .= 'includeFields=id,name,provider,category,images';
for ($j = 0; $j < count($pvArr); $j++) {
    $queryParam = 'includeFields=id,name,provider,category,images';
    $queryParam .= '&providers=' . $pvArr[$j];
    ...
    // file_put_contents('qt-launch.log', date('Y-m-d H:i:s') . ' targetURL : ' . $targetURL . PHP_EOL, FILE_APPEND);
    ...
    // exit();
    // $sql = "SELECT * FROM qt_game_list WHERE provider_id=?";
    // $res = $db->query($sql, $_GET['provider']);
    // $res = $db->query($sql);
    // if ($res->numRows() > 0) {
    //     $qtGameList = $res->fetchAll();
    // } else {
    for ($i = 0; $i < count($gameList['items']); $i++) {
        // if (isset($_GET['provider'])) {
        //     $sql = "SELECT * FROM qt_game_list WHERE game_id=? AND provider_id=?";
        //     $res = $db->query($sql, $gameList['items'][$i]['id'], $_GET['provider']);
        // } else {
...
if (isset($_GET['provider'])) {
    $sql = "SELECT * FROM qt_game_list WHERE provider_id=? AND isActive=1";
    $qtGameList = $db->query($sql, $_GET['provider'])->fetchAll();
} else {
    // $sql = "SELECT * FROM qt_game_list WHERE isActive=1 AND FIND_IN_SET('" . $_GET['gt'] . "',category)";
    $sql = "SELECT * FROM qt_game_list WHERE isActive=1 AND ";
    $sql .= "(category LIKE '%" . $_GET['gt'] . "%' OR ";
    $sql .= "category LIKE '%" . $_GET['gt'] . "' OR ";
    $sql .= "category LIKE '" . $_GET['gt'] . "%')";
    $qtGameList = $db->query($sql)->fetchAll();
}
// }
// for ($i = 0; $i < count($gameList['items']); $i++) {
//     for ($j = 0; $j < count($qtGameList); $j++) {
//         if ($qtGameList[$j]['game_id'] == $gameList['items'][$i]['id']) {
//             $gameList['items'][$i]['isActive'] = $qtGameList[$j]['isActive'];
//             $gameList['items'][$i]['username'] = $memberUsername;
//         }
//     }
// }

// echo json_encode($qtGameList);
exit();
                    ...
                    <div class="row">
                        ...
                        if (@sizeof($qtGameList) > 0) {
                            foreach ($qtGameList as $key => $value) {
                                ...
                                $sParam = "'" . $memberUsername . "','" . $value['game_id'] . "'";
                                ...
                                echo '<img src="' . $value['img_url'] . '" alt="" class="img-fluid game-img" style="width: 92%">';
                                echo '</a>';
