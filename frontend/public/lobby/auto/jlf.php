...
if (isset($_GET['lobby']) && $_GET['lobby'] == '1') {
    $memberUserName = $site['site_id'] . $_SESSION['member_no'];
    $mobile = 'false';
    ...
    $params = 'Token=' . $memberUserName . '&GameId=80&Lang=th-TH&AgentId=' . $agentId;
...
// $targetURL = "https://jili.apiplaza.com/GetGameList";
$targetURL = "https://jili.apiplaza.com/GetGameList?gt=all&selected=Fishing";
$res = $client->request('GET', $targetURL);  //<<<<<<<<<<<<<<<
$response =  $res->getBody()->getContents();
$jiliGameList = json_decode($response, true);
$userName = $site['site_id'] . $memberNo;
$imagePath = $site['services_url'] . '/jili-icon/GAMEID_';

for ($i = 0; $i < count($jiliGameList); $i++) {
    $sql = "SELECT game_id FROM jili_list_games WHERE game_id=?";
    $res = $db->query($sql, $jiliGameList[$i]['game_id']);
    if ($res->numRows() > 0) {
        // if ($jiliGameList[$i]['isActive'] != 1) {
        //     continue;
        // }
        $jiliGameList[$i]['gameIcon'] =  $imagePath . $jiliGameList[$i]['game_id'] . '_EN.png';
        $jiliGameList[$i]['username'] = $userName;
    } else {
        $sql = "INSERT INTO jili_list_games (game_id,name_en,name_cn,name_tw,game_category,game_type,isActive)";
        $sql .= " VALUES (?,?,?,?,?,?,?)";
        $db->query(
            $sql,
            $jiliGameList[$i]['game_id'],
            $jiliGameList[$i]['name_en'],
            $jiliGameList[$i]['name_cn'],
            $jiliGameList[$i]['name_tw'],
            $jiliGameList[$i]['game_category'],
            $jiliGameList[$i]['game_type'],
            1
        );
        $jiliGameList[$i]['gameIcon'] = $imagePath . $jiliGameList[$i]['game_id'] . '_EN.png';
        $jiliGameList[$i]['username'] = $userName;
...
<body style="margin:0;">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="projects" style="position: relative; height: 8858.81px;">
                    <div class="row">
                        <?php
                        if (@sizeof($jiliGameList) > 0) {
                            foreach ($jiliGameList as $key => $value) {
                                if (!array_key_exists('gameIcon', $value)) {
                                    continue;
                                }
                                $sParam = "'" . $value['username'] . "','" . $value['game_id'] . "'";
                                ...
                                echo '<a onclick="SelectGame(' . $sParam . ')" style="cursor: pointer;color: #ffc107;" class="game-title">' . $value['name_en'] .  '</a>';
                                echo '</div>';
                                echo '</div>';
                            }
                        } else {
    ...
    <script type="text/javascript">
        ...
        function SelectGame(username, game_code) {
            ...
            $.ajax({
                type: "POST",
                url: "/lobby/auto/authen-jili.php",
                dataType: "json",
                data: {
