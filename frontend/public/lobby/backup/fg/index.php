...
$res = $db->query("SELECT count(*) as count_all FROM fg_list_games WHERE gt='slot'")->fetchArray();
...
$res = $db->query("SELECT count(*) as count_all FROM fg_list_games WHERE gt='fruit'")->fetchArray();
$countECasino = $res['count_all'];
$res = $db->query("SELECT count(*) as count_all FROM fg_list_games WHERE gt='poker'")->fetchArray();
$countBingo = $res['count_all'];

$listGames = $db->query("SELECT * FROM fg_list_games")->fetchAll();
?>
<html lang="th">
<head>
      ...
      <div class="col-lg-12">
        <ul class="filters text-center" style="font-size: x-large;">
          <li id="all" class="active" data-filter="*">
            <a href="#!">
              <img src="<?php echo $site['host'] . '/lobby/fg/assets/img/all-icon.png' ?>" style="width: 100px;"><BR><b>เกมส์ทั้งหมด</b> (<?php echo $countAll ?>)
            </a>
          </li>
          <li data-filter=".Slot">
            <a href="#!">
              <img src="<?php echo $site['host'] . '/lobby/fg/assets/img/slot.png' ?>" style="width: 100px;"><BR><b>สล็อต</b> (<?php echo $countSlot ?>)
            </a>
          </li>
          ...
          <li data-filter=".ECasino">
            <a href="#!">
              <img src="<?php echo $site['host'] . '/lobby/fg/assets/img/casino.png' ?>" style="width: 100px;"><BR><b>Poker</b> (<?php echo $countECasino ?>)
            </a>
          </li>
          <li data-filter=".Bingo">
            <a href="#!">
              <img src="<?php echo $site['host'] . '/lobby/fg/assets/img/bingo.png' ?>" style="width: 100px;"><BR><b>ฟรุ๊ต</b> (<?php echo $countBingo ?>)
            </a>
          </li>
        </ul>
        ...
        <div class="projects" style="position: relative; height: 8858.81px;">
          <div class="row">
            ...
            foreach ($listGames as $key => $value) {
              ...
              // echo '<a class="game-item" href="' . $fgs_config['game_url'] . '/playGame?token=' . $member_login . '&appID=' . $fgs_config['app_id'] . '&gameCode=' . $value['game_code'] . '&language=' . $fgs_config['language'] . '&mobile=' . $mobile . '&redirectUrl=' . $redirect_url . '" target="_blank">';
              ...
              // echo '<a href="' . $fgs_config['game_url'] . '/playGame?token=' . $member_login . '&appID=' . $fgs_config['app_id'] . '&gameCode=' . $value['game_code'] . '&language=' . $fgs_config['language'] . '&mobile=' . $mobile . '&redirectUrl=' . $redirect_url . '" target="_blank" class="play-joker"><i class="lar la-play-circle"></i><b>เล่นเกมส์</b></a>';
              ...
              // echo '<a href="' . $fgs_config['game_url'] . '/playGame?token=' . $member_login . '&appID=' . $fgs_config['app_id'] . '&gameCode=' . $value['game_code'] . '&language=' . $fgs_config['language'] . '&mobile=' . $mobile . '&redirectUrl=' . $redirect_url . '" target="_blank" class="game-title">' . $value['game_name'] . '</a>';
