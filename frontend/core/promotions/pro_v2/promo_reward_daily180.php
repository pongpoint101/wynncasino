...
$sql = "SELECT 1 FROM log_deposit WHERE  member_no=? AND promo=180 AND trx_date=SUBDATE(CURDATE(),0);";
...
if ($res->num_rows() > 0) {
  ...
  $sql = "SELECT * FROM rewardcashback_daily WHERE member_no=? AND trx_date = SUBDATE(CURDATE(),1) AND status=1 ORDER BY id DESC LIMIT 1;"; 
  $res = $conn->query($sql,[$member_no]);
  if ($res->num_rows() <= 0) {
    $errCode = 402;
