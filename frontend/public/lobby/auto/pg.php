...
if (!$response) {
    ...
    $targetURL = "https://pgprod.apiplaza.com/GetGameList";
...
for ($i = 0; $i < count($PGGameList); $i++) {
    ...
    $sql = "SELECT id,status FROM pg_list_games_mix WHERE status<>0 AND id=? ORDER BY id DESC";
    $res = $db->query($sql, $PGGameList[$i]['id']);
    if ($res->numRows() > 0) {
        $res = $res->fetchArray();
        $PGGameList[$i]['gameIcon'] = $site['services_url'] . '/pg/' . $PGGameList[$i]['id'] . 'R.png';
        $PGGameList[$i]['status'] = $res['status'];
...
<body style="margin:0;">
                    ...
                    <div class="row">
                        ...
                        if (@sizeof($PGGameList) > 0) {
                            foreach ($PGGameList as $key => $value) {
                                ...
                                $sParam = "'" . $value['username'] . "','" . $value['id'] . "','" . $value['game_code'] . "','PGSOFT','" . $value['status'] . "'";
                            ...
                            // $RedirectURL = $site['host'] . $lobbyURL;
    ...
    <script type="text/javascript">
        ...
        function SelectGame(v_username, v_game_id, v_game_code, v_product, v_status) {
            let winOpen = window;
            let v_pg_select = 'soft';
            let v_platform_id = 21;
            let v_gc = v_game_id;
            if (v_status == 2) {
                v_pg_select = 'slot';
                v_platform_id = 122;
            ...
            $.ajax({
                type: "POST",
                url: "/lobby/auto/authen-pg" + v_pg_select + ".php",
                dataType: "json",
                data: {
