...
$KAConfig = json_decode($res['config_string'], true);
$targetURL = $KAConfig['api_url'] . '/gameList'; // 'http://api688.net/seamless/gameList';
$secretKey = $KAConfig['secret_key'];
$lang = "th";
$randomID = mt_rand(1, 999999999);
$arrBody = [
  'partnerName' => $KAConfig['partner_name'],
  'accessKey' => $KAConfig['access_key'],
  'language' => $lang,
  'randomeId' => $randomID
];

$hash = hash_hmac('SHA256', json_encode($arrBody), $secretKey);
$targetURL .= '?hash=' . $hash;
$arrFormParams['json'] = $arrBody;
...
$res = $client->request('POST', $targetURL, $arrFormParams);  //<<<<<<<<<<<<<<<
$KAGameListTemp = json_decode($res->getBody()->getContents(), true);
$getGameListStatus = $KAGameListTemp['status'];
$iconType = '&type=circular_framed';
// echo json_encode($KAGameListTemp, JSON_PRETTY_PRINT);
// exit();
for ($i = 0; $i < count($KAGameListTemp['games']); $i++) {
  $status = 1;
  $sql = "SELECT game_id,status FROM ka_list_games WHERE game_id=? ORDER BY id DESC";
  $res = $db->query($sql, $KAGameListTemp['games'][$i]['gameId']);
  if ($res->numRows() > 0) {
    $res = $res->fetchArray();
    $status = $res['status'];
  } else {
    $sql = "INSERT INTO ka_list_games (game_id,game_type,game_name,status,details) ";
    $sql .= "VALUES (?,?,?,?,?)";
    $db->query(
      $sql,
      $KAGameListTemp['games'][$i]['gameId'],
      $KAGameListTemp['games'][$i]['gameType'],
      $KAGameListTemp['games'][$i]['gameName'],
      1,
      json_encode($KAGameListTemp['games'][$i])
    );
  }
  $KAGameListTemp['games'][$i]['gameIcon'] = $KAGameListTemp['games'][$i]['iconURLPrefix'] . $iconType;
  $KAGameListTemp['games'][$i]['status'] = $status;
  $KAGameListTemp['games'][$i]['username'] = $memberUsername;
}
$KAGameList = array();
for ($i = 0; $i < count($KAGameListTemp['games']); $i++) {
  if ($KAGameListTemp['games'][$i]['popular'] == true) {
    array_push($KAGameList, $KAGameListTemp['games'][$i]);
  }
}
for ($i = 0; $i < count($KAGameListTemp['games']); $i++) {
  if ($KAGameListTemp['games'][$i]['popular'] == false) {
    array_push($KAGameList, $KAGameListTemp['games'][$i]);
  }
}
// echo json_encode($KAGameList);
// exit();
?>
...
<head>
  <title><?= $site['brand_name'] ?> - KA Gaming</title>
  <meta charset="utf-8">
    ...
    <div class="row">
      <div class="col-lg-12">
        ...
        <div class="projects" style="position: relative; height: 8858.81px;">
          <div class="row">
            <?php
            if (@sizeof($KAGameList) > 0) {
              foreach ($KAGameList as $key => $value) {
                ...
                $sParam = "'{$value['username']}','{$value['gameId']}'";
