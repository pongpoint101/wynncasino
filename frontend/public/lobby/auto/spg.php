...
// require_once './api/spg-function.php';
...
switch ($categoryID) {
  ...
  default:
    $gameType = 'slot'; //$categoryID=3
    $gamePrefix = 'S';
    break;
}

$sql = "SELECT * FROM m_providers WHERE provider_code='spg'";
$res = $db->query($sql)->fetchArray();
$spgConfig = json_decode($res['config_string'], true);
...
$targetURL = $spgConfig['api_url'];
$arr['body'] = json_encode([
  'merchantCode' => $spgConfig['merchant'],
  'serialNo' => genUUID()
]);
$res = $client->request('POST', $targetURL, $arr);  //<<<<<<<<<<<<<<<
$spgGameList = array();
$spgGameListTemp =  json_decode($res->getBody()->getContents(), true);
if ($spgGameListTemp['code'] != 0) {
  exit();
}
foreach ($spgGameListTemp['games'] as $key => $value) {
  $prefix = explode('-', $value['gameCode'])[0];
  if ($prefix != $gamePrefix) {
    continue;
  }
  array_push($spgGameList, $value);
}
// echo json_encode($spgGameList);
$i = 0;
$listnameshortshow = [];
foreach ($spgGameList as $key => $value) {
  $status = 1;
  $sql = "SELECT * FROM spg_list_games WHERE gameCode=?";
  $res = $db->query($sql, $value['gameCode']);
  if ($res->numRows() > 0) {
    $res = $res->fetchArray();
    $status = $res['isActive'];
  } else {
    $sql = "INSERT INTO spg_list_games (gameCode,gameName,gameType,thumbnail,screenshot,isActive) ";
    $sql .= "VALUES (?,?,?,?,?,?)";
    $db->query(
      $sql,
      $value['gameCode'],
  ...
  $spgGameList[$i]['gameIcon'] = $site['services_url'] . "/spg/$gameType/";
  $spgGameList[$i]['gameIcon'] .= str_replace(' ', '_', trim($value['gameName'])) . '.png';
  $spgGameList[$i]['status'] = $status;
  $spgGameList[$i]['username'] = $memberUsername;
  if (sizeof($listnameshortshow) <= 250) {
    $listnameshortshow['pgsindex' . $spgGameList[$i]['gameCode']] = strip_tags($PGGameList[$i]['gameName']);
  }

  $i++;
}
asort($spgGameList['gameName']);
// echo json_encode($spgGameList);
// exit();
...
<head>
  <title><?= $site['brand_name'] ?> - Spade Gaming</title>
  <meta charset="utf-8">
...
<body style="margin:0;">
  ...
  <div class="container">
    <div class="row mb-2 align-items-end">
      ...
      if (@sizeof($listnameshortshow) > 0) {
        ...
        $showtext = '';
        if (isset($_COOKIE['1gameselectSPG'])) {
          $showtext .= 'เช่น ' . $_COOKIE['1gameselectSPG'];
        }
        if (isset($_COOKIE['2gameselectSPG'])) {
          if ($showtext == '') {
            $showtext .= 'เช่น ' . $_COOKIE['2gameselectSPG'];
          } else {
            $showtext .= ',' . $_COOKIE['2gameselectSPG'];
          }
        }
        if ($showtext == '') {
  ...
  <script type="text/javascript">
    ...
    function SelectGame(v_username, v_game_code) {
      let winOpen = window;
