<?php 
 $sql = "SELECT * FROM promo_casino_60P WHERE member_no=?";
 ...
 if ($res->num_rows() > 0) {
     ...
     if ($res->num_rows() >= 1) {
         ...
         $conn->query("UPDATE log_deposit SET promo=" . $promo_id . " WHERE id=" . $res['id']);
         $expect_turnover = ($latest_deposit_amount + $promo_cal_result) * 20;  // Turnover x 20
         ...
         $conn->insert('promo_casino_60p', $data); 
     } else {
         $errCode = 402;
