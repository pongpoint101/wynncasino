...
if (strlen($data) > 0) {
 ...
 $sql .= " WHERE amount>0 AND status=1 AND YEAR(trx_date)=?";
 ...
 $deposit_today=GetDataSqlWhereOne($datakey,$sql,[$year],5*60);   
 
 $sql = "SELECT * From log_withdraw";
 $sql .= " WHERE amount>0 AND status=1 AND YEAR(trx_date)=?";
 
 $datakey='report_withdraw:'.$trx_date.':'.$site['brand_name']; 
 $res=GetDataSqlWhereAll($datakey,$sql,[$year],5*60);    
 ...
 // New register member
 $sql = "SELECT count(1) as registered_today FROM members WHERE  YEAR(create_at)=?";
 $datakey='report_regis_today:'.$trx_date.':'.$site['brand_name']; 
 $register_member=GetDataSqlWhereOne($datakey,$sql,[$year],5*60);   
