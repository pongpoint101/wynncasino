...
$qmConfig = getProviderConfig('km', 66, 1);
$headers = [
    'exceptions'       => false,
    'verify'           => false,
    'headers' => [
...
for ($i = 0; $i < count($response['games']); $i++) {
    $prepareArray = false;
    if ($response['games'][$i]['isactive'] != 1 || $response['games'][$i]['type'] != 1) {
        continue;
    }
    $sql = "SELECT game_id,isActive FROM qmkm_game_list ";
    $sql .= "WHERE game_id=?";
    ...
    if ($res->numRows() > 0) {
        ...
        $sql = "INSERT INTO qmkm_game_list (game_id,game_code,game_name,game_json) ";
        $sql .= "VALUES (?,?,?,?)";
        // echo $sql . '<br>';
        try {
            $db->query(
    ...
    if ($prepareArray) {
        $response['games'][$i]['iconurl'] = $site['services_url'] . "/qm/km/Game_KMQM_" . $response['games'][$i]['code'] . '_520x520.jpg';
        $response['games'][$i]['iconurl_local'] = "http://services.12pay.local/qm/km/Game_KMQM_" . $response['games'][$i]['code'] . '_520x520.jpg';
...
<body style="margin:0;">
    ...
    <div class="container">
        <a href="/lobby">
            <img src="<?= $site['services_url'] . '/' . $resLobby['img_path'] . '/' . $resLobby['img_logo'] ?>" style="width: 50%;">
        </a>
    </div>
                    ...
                    <div class="row">
                        ...
                        if (count($gameList['games']) > 0) {
                            foreach ($gameList['games'] as $key => $value) {
                                ...
                                $sParam = "'" . $value['username'] . "','" . $value['code'] . "','KM'";
